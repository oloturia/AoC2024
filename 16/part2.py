...
def main(input_file):
	...
	dijks = DijkstraSPF(graph,start_pos)
	mindist = dijks.get_distance(end_pos)
	nodes = graph.get_nodes()
	steps = set()
	
	for n in nodes:
		if len(graph.get_adjacent_nodes(n)) > 2:
			from_start = dijks.get_distance(n)
			if from_start >= mindist:
				continue
			t_dij = DijkstraSPF(graph,n)
			to_end = t_dij.get_distance(end_pos)
			if from_start+to_end == mindist:
				steps = steps.union( set(t_dij.get_path(end_pos) )  )
				steps = steps.union( set(dijks.get_path(n) ) )
